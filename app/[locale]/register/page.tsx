      ...
      <main className="flex-1 flex flex-col justify-center px-6 pb-24 relative z-10">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="max-w-md w-full mx-auto">
          <div className="mb-10 text-center">
            <h1 className="text-4xl font-black text-slate-800 mb-2 tracking-tight">Create Account</h1>
            <p className="text-slate-500">Join us to start generating and saving delicious recipes.</p>
          </div>
          ...
          <form onSubmit={handleRegister} className="bg-white rounded-[2rem] p-8 shadow-xl shadow-slate-200/50 border border-slate-100 space-y-5">
            ...
            <div className="space-y-4">
              <div className="relative">
                ...
                <input type="text" placeholder="Full Name" value={name} onChange={(e) => setName(e.target.value)} required className="w-full pl-11 pr-4 py-4 bg-slate-50 border border-slate-100 rounded-2xl outline-none focus:ring-2 focus:ring-[var(--primary)] focus:bg-white transition-all text-slate-700 placeholder:text-slate-400" />
              </div>
